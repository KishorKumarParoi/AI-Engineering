...
# gcloud projects describe enterprise-rag --format="value(YOUR_PROJECT_NUMBER)"
...
gcloud storage buckets create gs://YOUR_PROJECT_ID-rag-raw --location=us-central1
gcloud storage buckets create gs://YOUR_PROJECT_ID-rag-processed --location=us-central1
```
...
gcloud builds submit --tag us-central1-docker.pkg.dev/YOUR_PROJECT_ID/rag-repo/rag-api:v1 .
```
...
gcloud run deploy rag-api \
  --image us-central1-docker.pkg.dev/YOUR_PROJECT_ID/rag-repo/rag-api:v1 \
  --region us-central1 \
  --platform managed \
  ...
  --set-env-vars "GCP_DOC_AI_PROCESSOR_ID=your_doc_ai_processor_id" \
  --set-env-vars "GCP_RAW_BUCKET=your-raw-bucket-name" \
  --set-env-vars "GCP_PROCESSED_BUCKET=your-processed-bucket-name" \
  --set-env-vars "QDRANT_API_KEY=your_qdrant_api_key" \
  --set-env-vars "QDRANT_CLUSTER_ENDPOINT=your_qdrant_cluster_endpoint" \
  --set-env-vars "GROQ_API_KEY=your_groq_api_key" \
  --set-env-vars "LOGFIRE_TOKEN=your_logfire_token" \
  --set-env-vars "LANGSMITH_TRACING=true" \
  --set-env-vars "LANGSMITH_PROJECT=enterprise_rag" \
  --set-env-vars "LANGSMITH_API_KEY=your_langsmith_api_key" \
...
1. **The Ingestion Test**: Upload a PDF to your raw GCS bucket (`gs://YOUR_PROJECT_ID-rag-raw`). Check the Cloud Run logs for the `ingestion` service—you should see it wake up automatically!
