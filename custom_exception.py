...
class CustomException(Exception):
    ...
    Attributes:
        error_message (str): Plain string message of the error.
        detailed_message (str): Error message augmented with line number and file trace.
        error_code (str): Machine-readable string code (e.g., 'ERR_DATA_VALIDATION').
        status_code (int): Corresponding HTTP status code (e.g., 400, 404, 500).
        details (Dict[Any, Any]): Additional context/metadata dictionary.
    ...
    def __init__(
        ...
        """
        Initialize CustomException with rich metadata.

        Args:
            error_message (Union[Exception, str]): Descriptive error message or original exception.
            error_detail (Optional[Any]): sys module or traceback context for frame extraction.
            error_code (Optional[str]): Unique machine-readable error code string.
            status_code (Optional[int]): HTTP status code (default: 500).
            details (Optional[Dict[str, Any]]): Contextual key-value pairs for debugging/logging.
        """
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            try:
                ...
                # Re-raise already structured custom exceptions
                raise
...
if __name__ == "__main__":
    print("--- Demonstrating Industry Standard Custom Exception --- \n")

    # Example 1: Standard Catch and Wrap
    try:
        result = 10 / 0
    except Exception as e:
        custom_err = CustomException(
            error_message="Division by zero encountered in calculation.",
            error_detail=sys,
            details={"dividend": 10, "divisor": 0}
        )
        print("1. Standard String Output:")
        print(f"   {custom_err}\n")
        print("2. JSON-Serializable Structured Output (`to_dict()`):")
        import json
        print(json.dumps(custom_err.to_dict(), indent=2))
        print()

    # Example 2: Domain Specific Exception (Validation Error)
    try:
        user_age = -5
        if user_age < 0:
            raise DataValidationError(
                error_message="User age cannot be negative.",
                error_detail=sys,
                details={"provided_age": user_age, "field": "age"}
            )
    except DataValidationError as e:
        print("3. Domain-Specific Exception Caught:")
        print(f"   Code: {e.error_code}, Status: {e.status_code}")
        print(f"   Message: {e.raw_message}")
        print(f"   Trace: File '{e.file_name}', Line {e.line_number}\n")

    # Example 3: Using Decorator
    @handle_custom_exception(wrapper_exception=ModelInferenceError, default_message="Failed to execute model pipeline.")
    def run_inference(data_input: str):
        raise ValueError("Invalid embedding dimension length.")

    try:
        run_inference("sample text")
    except ModelInferenceError as e:
        print("4. Decorator Wrapped Exception Caught:")
        print(f"   {e}")
        print(f"   Context: {e.details}")

    print("\n--- Custom Exception Demo Completed Successfully ---")
